...
const char* serverName = "http://192.168.1.195/bike_rental/log_rental.php"; // Replace with your XAMPP server IP
...
void loop() {
  ...
  if (rfidTag != "") { // Check if a tag was read
    ...
    if (isAuthorizedTag(rfidTag)) { // Check if the tag is authorized
      toggleSolenoid();
      logToServer(rfidTag);
    } else {
      Serial.println("Unauthorized RFID Tag");
      playUnauthorizedSound(); // Play broken beep pattern
    }
  }
}
...
void playUnauthorizedSound() {
  // Broken beep pattern for unauthorized tag
  for (int i = 0; i < 60; i++) {  // Play 5 broken beeps
    digitalWrite(BUZZER_PIN, HIGH);
    delay(100); // Short beep duration
    digitalWrite(BUZZER_PIN, LOW);
    delay(100); // Short pause between beeps
...
void logToServer(String rfidTag) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    String serverPath = String(serverName) + "?tag=" + rfidTag;

    http.begin(serverPath.c_str());
    int httpResponseCode = http.GET();
    //int httpRespondCode = http.POST();
